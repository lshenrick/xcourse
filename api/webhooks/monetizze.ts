...
// 2 = Finalizada (Approved)
// 3 = Cancelada
// 4 = Devolvida (Refunded)
// 5 = Em disputa
// 6 = Completa
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Monetizze sends data in nested objects: produto, comprador, venda
  const productId = payload?.produto?.codigo?.toString() || payload?.product?.code?.toString();
  const buyerEmail = payload?.comprador?.email || payload?.customer?.email;
  const buyerName = payload?.comprador?.nome || payload?.customer?.name;
  const transactionCode = payload?.venda?.codigo?.toString() || payload?.transaction?.code?.toString();
  const statusCode = Number(payload?.venda?.status || payload?.transaction?.status || 0);

  const logEntry = {
    event_type: `monetizze_status_${statusCode}`,
    buyer_email: buyerEmail || null,
    buyer_name: buyerName || null,
    product_id: productId || null,
    transaction_id: transactionCode || null,
    raw_payload: payload,
  ...
  try {
    if (!buyerEmail || !productId) {
      await supabase.from("webhook_logs").insert({
        ...logEntry,
        status: "error",
        error_message: "Missing required fields: comprador.email or produto.codigo",
      });
    ...
    // Validate token
    const monetizzeToken = req.headers["x-monetizze-token"] as string
    ...
    for (const mapping of productMapping) {
      ...
      if ((settings as any)?.monetizze_token && monetizzeToken !== (settings as any).monetizze_token) {
        await supabase.from("webhook_logs").insert({
          ...logEntry,
          area_slug: areaSlug,
          status: "error",
          error_message: "Invalid Monetizze token",
        });
        continue;
      }
      ...
      // Status 2 (Finalizada) or 6 (Completa) = grant access
      if (statusCode === 2 || statusCode === 6) {
        const finalEmail = buyerEmail.toLowerCase().trim();
        const finalName = buyerName || null;
          ...
          .upsert(
            {
              ...
              name: finalName,
              area_slug: areaSlug,
              payment_provider: "monetizze",
              monetizze_transaction: transactionCode || null,
              monetizze_product_id: productId,
              status: "active",
      ...
      // Status 3 (Cancelada) or 4 (Devolvida) = revoke access
      } else if (statusCode === 3 || statusCode === 4) {
        const revokeStatus = statusCode === 4 ? "refunded" : "revoked";
        ...
        await supabase.from("webhook_logs").insert({
          ...
          area_slug: areaSlug,
          status: "ignored",
          error_message: `Monetizze status ${statusCode} not handled`,
        });
      }
    }
